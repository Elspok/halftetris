...
#The grid indicates where the middle of each square of each shape should be
            ...
            for old_rect in shape.get_rects():
                for square in self.squares:
                    if pygame.Rect.colliderect(square, old_rect):
                        return True 
        return False
#so if the space between is 20 the size of each square is 40x40
grid = Grid(20,1, (60, 179, 113))


#Here all of the shapes that had already falled to the floor will be saved
random_old = []
#cycle cound and lists of shapes and rotations
cycles = 0
coordinates = [T_COOR,O_COOR,I_COOR,L_COOR,S_COOR]
rotations = [0,90,180,270]
while running == True:
    #first thing, fill screen with white, show grid
    screen.fill("white")
    grid.display()    
    #At cicle 0 we create a random shape and give it a random position 
    if cycles == 0:
        
        random_coor = coordinates[random.randint(0,len(coordinates)-1)]
        rotation = rotations[random.randint(0,len(rotations)-1)]
        random_shape = Shape(grid.horizontal_positions,grid.vertical_positions, (0,0,0), random_coor)    
        if rotation != 0: 
            random_shape = Shape(grid.horizontal_positions,grid.vertical_positions, (0,0,0), random_shape.rotate(rotation))                                 
        ...
        while random_shape.on_vertical_limit():
            ...
            random_shape = Shape(grid.horizontal_positions,grid.vertical_positions, (0,0,0), new_positions)              
    #Give a movement to the shape
    #MAKE SPEED SLOWER  
        ...
        if random_shape.on_vertical_limit() == False:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    random_shape = Shape(grid.horizontal_positions,grid.vertical_positions, (0,0,0), random_shape.rotate(90))                    
    
    #Check for collitions
    #All of the old shapes need to check if colided with the other shapes
    #notiece each shape has four rects (rectangular shapes made by pygame)
    random_shape.display()
    
    if random_shape.on_vertical_limit() or random_shape.get_colitions(random_old):
        print("in")
